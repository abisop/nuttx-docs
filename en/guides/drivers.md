...
modification.
  ...
  - Find a similar driver in NuttX source code:
      ...
      - Search the NuttX codebase for the register names (try several
        different ones).
      - Note that you'll have to compare the datasheet to the header and
        code files to see if there are differences; there will usually
        be some differences between architectures, and they can be
        significant.
  ...
  - Understanding how the driver works
    ...
    >         running `make menuconfig` (that will reorder the file,
    ...
    >     commandline, but have also used it from an IDE like JetBrains'
    >     Clion, where it's easier to see the code context.
    >     
...
If you're not porting a NuttX driver from another architecture, it still
helps to look at other similar NuttX drivers, if there are any. For
instance, when implementing an Ethernet driver, look at other NuttX
Ethernet drivers; for an SD Card driver, look at other NuttX SD Card
drivers. Even if the chip-specific code won't be the same, the structure
to interface with NuttX can be used.
...
## DMA Debugging
  ...
  - Remember that logging can change the timing of any algorithms you
    ...
    added or removed. Definitely test with logging disabled.
