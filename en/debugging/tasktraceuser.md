...
The following configurations are configurable parameters for trace.
  ...
  - `CONFIG_DRIVERS_NOTERAM_CRASH_DUMP`
      - If enabled, it will dump the data in the noteram buffer after a
        system crash. This function can help to view the behavior of the
...
\[<span class="title-ref">../application\](</span>../application.md)s/nsh/builtin\`
will be available.
...
By using the logging function of your terminal software, the trace
...
for "Trace Compass" by transferring the file in the target device to the
host.
...
Wildcard "`*`" can be used to specify the system call name. For example,
"`trace syscall +sem_*`" filters the system calls begin with "`sem_`",
such as `sem_post()`, `sem_wait()`,...
