...
initialize by calling :c`sam_afec_initialize`

Software trigger can be selected by `CONFIG_SAMV7_AFECn_SWTRIG`,
...
initialize by calling :c`sam_dac_initialize`.
...
The lower-half of the peripheral is initialized by function
:c`sam_mcan_initialize`
...
The lower-half is initialized by function :c`sam_pwminitialize`.
...
The lower-half is initialized by function :c`sam_qspi_initialize` in
case of QSPI mode and by :c`sam_qspi_spi_initialize` in case of SPI
mode.
...
The peripheral is initialized by :c`sam_spibus_initialize` function.
...
The controller is initialized by :c`sam_ssc_initialize` function.
...
have this timeout support, therefore function :c`sam_serial_dma_poll`
should be called periodically to flush the DMA buffers. Boards can use
common :c`board_uart_rxdma_poll_init` function to initialize a timer
triggering the poll.
...
SPI master (host) mode is enabled by `CONFIG_SAMV7_USARTx_SPI_MASTER`
...
initialized by :c`sam_serial_spi_initialize` with port number as an
argument.
...
is initialized by :c`sam_1wireinitialize` with port number as an
argument.
